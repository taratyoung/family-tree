...
const Relationship = require('../db').import('../models/relationship');

//add post for relationships
//add a put and delete for relationships

router.post('/connect', async (req, res) => {
    const relationshipId = req.body.relationshipId
    const person1Id = req.body.person1Id
    const person2Id = req.body.person2Id
    const relationship = await Relationship.findOne({ where: { id: relationshipId } })
    const result1 = await relationship.addPerson(person1Id)
    const result2 = await relationship.addPerson(person2Id)
